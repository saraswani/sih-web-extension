...
(function() {
  ...
  class CanvasPixelRedactor {
    ...
    applyGaussianBlur(ctx, x, y, width, height, blurRadius = 20) {
      ...
      // Draw subtle dark mask and orange pill badge matching Image 2
      ctx.save();
      ...
      const badgeW = 100;
      const badgeH = 20;
      const badgeX = x + Math.max(0, (width - badgeW) / 2);
      const badgeY = y + Math.max(0, (height - badgeH) / 2);
      ...
      ctx.fillText('[FACE_BLURRED]', badgeX + badgeW / 2, badgeY + badgeH / 2 + 1);
     ...
     */
    async redactScreenshot(rawScreenshotDataUrl, domBoxes = [], faceBoxes = []) {
