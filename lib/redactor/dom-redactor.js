    ...
    redactDOMFaces(faceBoxes = []) {
      ...
      for (const face of faceBoxes) {
        ...
        overlay.style.cssText = `
          ...
          width: ${Math.max(75, fw)}px !important;
          height: ${Math.max(45, fh)}px !important;
          z-index: 2147483640 !important;
          backdrop-filter: blur(16px) contrast(150%) brightness(70%) !important;
          -webkit-backdrop-filter: blur(16px) contrast(150%) brightness(70%) !important;
          background: radial-gradient(circle, rgba(0, 0, 0, 0.92) 0%, rgba(15, 23, 42, 0.85) 100%) !important;
          border: 1px solid rgba(217, 107, 67, 0.4) !important;
        ...
        `;
        badge.textContent = '[FACE_BLURRED]';

        overlay.appendChild(badge);

        if (face.element && face.element.style) {
          face.element.dataset.psOriginalFilter = face.element.style.filter || '';
          face.element.style.filter = 'blur(12px) contrast(130%)';
        }
        ...
        overlay.addEventListener('click', (e) => {
          ...
          if (isRevealed) {
            if (face.element && face.element.style) {
              face.element.style.filter = face.element.dataset.psOriginalFilter || 'none';
            }
            ...
            overlay.style.backdropFilter = 'blur(16px) contrast(150%) brightness(70%)';
            overlay.style.webkitBackdropFilter = 'blur(16px) contrast(150%) brightness(70%)';
            overlay.style.background = 'radial-gradient(circle, rgba(0, 0, 0, 0.92) 0%, rgba(15, 23, 42, 0.85) 100%)';
            overlay.style.borderColor = 'rgba(217, 107, 67, 0.4)';
            badge.style.background = '#d96b43';
            badge.style.color = '#ffffff';
            badge.textContent = '[FACE_BLURRED]';
          }
        });
        ...
        this.faceOverlays.push({ overlay, element: face.element });
      }
    }

    /**
     * Clears all active DOM face overlays and restores image filters.
     */
    clearDOMFaceOverlays() {
      if (this.faceOverlays && this.faceOverlays.length > 0) {
        for (const item of this.faceOverlays) {
          const el = item.overlay || item;
          const targetImg = item.element;
          if (targetImg && targetImg.dataset && targetImg.dataset.psOriginalFilter !== undefined) {
            targetImg.style.filter = targetImg.dataset.psOriginalFilter;
            delete targetImg.dataset.psOriginalFilter;
          }
          if (el && el.parentNode) {
